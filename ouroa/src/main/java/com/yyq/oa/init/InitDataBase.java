...
//                new ClassPathXmlApplicationContext("applicationContext.xml", "spring-activiti.xml");
//        ProcessEngine processEngine = (ProcessEngine) context.getBean("processEngine");
    ...
    public static void importSql(Properties properties) throws IOException {
        ...
        FileOutputStream fos = new FileOutputStream(Resources.getResourceURL("import/importdb.properties").getFile());
    ...
    private static String[] getImportCommand(Properties properties) {
        ...
        Lg.log("commands", commands);
        return commands;
    }
    
    private static boolean isInitialized(Properties properties) {
        return !properties.getProperty("status").startsWith("0");
    }
}
